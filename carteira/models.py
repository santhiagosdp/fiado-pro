...
from django.conf import settings  # + NEW (opcional, para deleted_by)


class Empresa(models.Model):
    owner = models.OneToOneField(User, on_delete=models.CASCADE, related_name="empresa")
    nome = models.CharField(max_length=150)
    cnpj_cpf = models.CharField(max_length=20, blank=True) #até 18, numeros e tracos
...
class ContaCarteira(models.Model):
    ...
    # --- SOFT DELETE (NOVOS CAMPOS) ---
    is_deleted = models.BooleanField(default=False)
    deleted_at = models.DateTimeField(null=True, blank=True)
    deleted_reason = models.CharField(max_length=255, blank=True)
    deleted_by = models.ForeignKey(
        User, null=True, blank=True, on_delete=models.SET_NULL, related_name="contas_carteira_excluidas"
    )
...
class Pagamento(models.Model):
    ...
    def __str__(self):
        return f"Pgto {self.valor} em {self.data:%d/%m/%Y %H:%M}"
...
class AuditLog(models.Model):
    ...
    descricao = models.TextField()  # ex.: "Excluiu conta #4 (motivo: ...)"
